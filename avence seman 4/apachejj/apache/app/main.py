...
def pinot_query(sql: str):
    try:
        # Usamos requests directamente al Broker SQL endpoint (más fiable que pinotdb)
        broker_url = f"http://{PINOT_HOST}:8099/query/sql"
        r = requests.post(broker_url, json={"sql": sql}, timeout=10)
        
        if r.status_code != 200:
            logger.error(f"Error de Pinot ({r.status_code}): {r.text}")
            raise HTTPException(status_code=500, detail=f"Pinot error: {r.text}")

        data = r.json()
        if "resultTable" not in data:
            logger.warning(f"Pinot no devolvió resultTable: {data}")
            return []

        rt = data["resultTable"]
        raw_cols = rt.get("columnNames", [])
        rows = rt.get("rows", [])

        # 1. Minúsculas 2. Quitar prefijos de tabla (ej: "vehiculos.make" -> "make")
        clean_cols = [c.lower().split('.')[-1] for c in raw_cols]
        ...
        logger.error(f"Excepción en pinot_query: {e}")
